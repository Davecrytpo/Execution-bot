...
function deriveWsUrl(rpcUrl: string) {
  if (!rpcUrl) {
    return '';
  }
...
export const config = {
  ...
  sniperLogQueueMax: toNumber(process.env.SNIPER_LOG_QUEUE_MAX, 30, 1),
  sniperLogProcessIntervalMs: toNumber(process.env.SNIPER_LOG_PROCESS_INTERVAL_MS, 1500, 0),
  sniperCreateProcessIntervalMs: toNumber(process.env.SNIPER_CREATE_PROCESS_INTERVAL_MS, 15000, 0),
  ...
  sniperMinInitialLiquiditySol: toNumber(process.env.SNIPER_MIN_INITIAL_LIQUIDITY_SOL, 1, 0),
  ...
  sniperMinUniqueBuyerRatio: toNumber(process.env.SNIPER_MIN_UNIQUE_BUYER_RATIO, 0.3, 0),
  sniperMaxSuspiciousWallets: toNumber(process.env.SNIPER_MAX_SUSPICIOUS_WALLETS, 3, 0),
  sniperRequireEarlyBuyInterest: toBoolean(process.env.SNIPER_REQUIRE_EARLY_BUY_INTEREST, true),
  sniperRequireWalletDiversity: toBoolean(process.env.SNIPER_REQUIRE_WALLET_DIVERSITY, true),
  sniperAllowRiskyBuys: toBoolean(process.env.SNIPER_ALLOW_RISKY_BUYS, false),
  sniperRiskyBuyMinScore: toNumber(process.env.SNIPER_RISKY_BUY_MIN_SCORE, 50, 0),
